...
logger.info("Γ£à All Microsoft Agent Framework agents initialized")
logger.info(f"Γ£à Azure OpenAI configured: {Config.AZURE_OPENAI_API_KEY is not None}")
logger.info("="*60)
...
async def analyze_pipeline(request: PipelineAnalysisRequest):
    ...
    try:
        ...
        logger.info("="*60)
        logger.info(f"Γ£à Analysis complete: {report['summary']['decision']}")
        logger.info(f"Γ£à AI Enhanced: {report['ai_enhanced']}")
        logger.info("="*60)
        ...
        logger.error(f"Γ¥î Analysis failed: {str(e)}")
...
async def generate_report(request: PipelineAnalysisRequest):
    ...
    try:
        ...
        violations_html = "".join(f"<li>Γ¥î {v}</li>" for v in policy_result.get("violations", []))
        warnings_html = "".join(f"<li>ΓÜá∩╕Å {w}</li>" for w in policy_result.get("warnings", []))
        passed_html = "".join(f"<li>Γ£à {p}</li>" for p in policy_result.get("passed", []))
        risk_factors_html = "".join(
            f"<li><strong>{f['factor']}</strong> ΓÇö severity: {f['severity']}</li>"
...
<body>
  <div class="header">
    <h1>≡ƒî▒ GreenOps AI ΓÇö Compliance Report</h1>
  ...
  <div class="container">
    ...
    <div class="grid">
      <div class="card">
        ...
        <div class="sub">Rating: {carbon_result.get('rating', 'N/A')} ┬╖ {carbon_result.get('co2_grams', 0)} g COΓéé</div>
      </div>
      <div class="card">
    ...
    {"<div class='section'><h3>ΓÜá∩╕Å Risk Factors</h3><ul>" + risk_factors_html + "</ul></div>" if risk_factors_html else ""}

    {"<div class='section'><h3>≡ƒÆí Recommendations</h3><ul>" + recommendations_html + "</ul></div>" if recommendations_html else ""}
...
if __name__ == "__main__":
    import uvicorn
    logger.info("="*60)
    logger.info("≡ƒÜÇ Starting GreenOps AI with Microsoft Agent Framework")
    logger.info("≡ƒÅå AI Dev Days Hackathon 2026")
    logger.info(f"Γ£à Azure OpenAI: {'Configured' if Config.AZURE_OPENAI_API_KEY else 'Fallback Mode'}")
    logger.info("="*60)
